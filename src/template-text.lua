    ...
    for line in lines(template) do
        ...
        do
          ...
          if tableVarName ~= nil then
              ...
              -- Thus we copy the line as it is
                  lineOfCode = string.format("table.insert(text, %q)",
                      tableIndent .. slashes.actual_chars .. "${" .. tableVarName .. "}" .. trailingSpace)
              elseif tableVarName == "" then
                  -- we have an empty argument, i.e. '${}' - preserve the blanks
                  lineOfCode = string.format("table.insert(text, %q)", indent .. tableIndent .. trailingSpace)
              else
...
local function tload(template, opts, env, included_templates)
  ...
  expanded -- just to have the same ret values as in the error case
end
